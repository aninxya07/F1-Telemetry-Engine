...
class F1ReplayWindow(arcade.Window):
    def __init__(self, frames, track_statuses, example_lap, drivers, title,
        ...
        # Load car images from images/drivers/ folder
        self._car_textures = {}
        cars_folder = os.path.join("images", "drivers")
        if os.path.exists(cars_folder):
            for filename in os.listdir(cars_folder):
                if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                    driver_code = os.path.splitext(filename)[0].upper()
                    texture_path = os.path.join(cars_folder, filename)
                    try:
                        self._car_textures[driver_code] = arcade.load_texture(texture_path)
                    except Exception as e:
                        print(f"Warning: Could not load car image {texture_path}: {e}")
    ...
    def on_draw(self):
        ...
        for code, pos in frame["drivers"].items():
            ...
            # Highlight selected driver with larger size and white outline
            if code == self.selected_driver:
                # Draw white outline first
                arcade.draw_circle_outline(sx, sy, 10, arcade.color.WHITE, 3)
                # Draw larger filled circle
                arcade.draw_circle_filled(sx, sy, 8, color)
            else:
                # Normal size for unselected drivers
        ...
        if self.selected_driver and self.selected_driver in frame["drivers"]:
            ...
            box_width = 300
            box_height = 200  # Increased to accommodate driver image
            ...
            # Car Image
            car_texture = self._car_textures.get(self.selected_driver.upper())
            if car_texture:
                image_size = 80
                image_x = info_x + box_width - image_size - 10
                image_y = info_y - 30
                image_rect = arcade.XYWH(
    ...
    def on_mouse_press(self, x: float, y: float, button: int, modifiers: int):
        # First check if click is on leaderboard
        new_selection = None
        ...
        # If not on leaderboard, check if click is on a car dot
        if new_selection is None:
            ...
            for code, pos in frame["drivers"].items():
                sx, sy = self.world_to_screen(pos["x"], pos["y"])
                # Calculate distance from click to car position
                dx = x - sx
                dy = y - sy
        ...
        # Toggle if clicking the same entry/dot
        if new_selection == self.selected_driver:
            self.selected_driver = None
        else:
            self.selected_driver = new_selection

class SelectionMenuWindow(arcade.Window):
    def __init__(self, start_callback=None):
        super().__init__(SCREEN_WIDTH, SCREEN_HEIGHT, "F1 Race Replay - Selection Menu", resizable=False)
        ...
        # UI state
        self.year_dropdown_open = False
        self.round_dropdown_open = False
        
        # Available options
        self.years = list(range(2018, 2026))  # 2018-2025
        self.rounds = list(range(1, 25))  # 1-24
        
        # Button/dropdown positions
        self.center_x = SCREEN_WIDTH // 2
        self.center_y = SCREEN_HEIGHT // 2
        
        # Initialize rects (will be set in on_draw)
        self.year_btn_rect = None
        self.round_btn_rect = None
        self.race_btn_rect = None
        self.sprint_btn_rect = None
    ...
    def on_draw(self):
        ...
        year_btn_width = 150
        year_btn_height = 40
        year_btn_x = self.center_x - 150
        year_btn_rect = arcade.XYWH(
            year_btn_x,
            year_y,
            year_btn_width,
            year_btn_height
        )
        arcade.draw_rect_filled(year_btn_rect, arcade.color.GRAY)
        arcade.draw_rect_outline(year_btn_rect, arcade.color.WHITE, 2)
        arcade.Text(
            str(self.year),
            year_btn_x,
            year_y,
        ...
        # Year dropdown list
        if self.year_dropdown_open:
            dropdown_y_start = year_y - year_btn_height / 2 - 5
            for i, year_option in enumerate(self.years):
                option_y = dropdown_y_start - (i * 35) - 20
                option_rect = arcade.XYWH(
                    year_btn_x,
                    option_y,
                    year_btn_width,
                    30
                )
                color = arcade.color.DARK_GRAY if year_option == self.year else arcade.color.LIGHT_GRAY
                arcade.draw_rect_filled(option_rect, color)
                arcade.draw_rect_outline(option_rect, arcade.color.WHITE, 1)
                arcade.Text(
                    str(year_option),
                    year_btn_x,
                    option_y,
        ...
        round_btn_width = 150
        round_btn_height = 40
        round_btn_x = self.center_x - 150
        round_btn_rect = arcade.XYWH(
            round_btn_x,
            round_y,
            round_btn_width,
            round_btn_height
        )
        arcade.draw_rect_filled(round_btn_rect, arcade.color.GRAY)
        arcade.draw_rect_outline(round_btn_rect, arcade.color.WHITE, 2)
        arcade.Text(
        ...
        if self.round_dropdown_open:
            dropdown_y_start = round_y - round_btn_height / 2 - 5
            for i, round_option in enumerate(self.rounds):
                option_y = dropdown_y_start - (i * 35) - 20
                # Only show if it's on screen
                if option_y > 50:  # Leave some margin at bottom
                    option_rect = arcade.XYWH(
                        round_btn_x,
                        option_y,
                        round_btn_width,
                    ...
                    color = arcade.color.DARK_GRAY if round_option == self.round_number else arcade.color.LIGHT_GRAY
                    arcade.draw_rect_filled(option_rect, color)
                    arcade.draw_rect_outline(option_rect, arcade.color.WHITE, 1)
                    arcade.Text(
                        str(round_option),
        ...
        # Session Type Selection (Race/Sprint)
        session_y = self.center_y + 10
        arcade.Text(
            "Session:",
            self.center_x - 200,
            session_y,
        ...
        race_btn_width = 120
        race_btn_height = 40
        race_btn_x = self.center_x - 150
        race_btn_rect = arcade.XYWH(
            race_btn_x,
            session_y,
            race_btn_width,
            race_btn_height
        )
        race_color = arcade.color.GREEN if self.session_type == 'R' else arcade.color.GRAY
        arcade.draw_rect_filled(race_btn_rect, race_color)
        arcade.draw_rect_outline(race_btn_rect, arcade.color.WHITE, 2)
        arcade.Text(
            "Race",
            race_btn_x,
            session_y,
        ...
        # Sprint button
        sprint_btn_x = self.center_x - 10
        sprint_btn_rect = arcade.XYWH(
            sprint_btn_x,
            session_y,
            race_btn_width,
            race_btn_height
        )
        sprint_color = arcade.color.ORANGE if self.session_type == 'S' else arcade.color.GRAY
        arcade.draw_rect_filled(sprint_btn_rect, sprint_color)
        arcade.draw_rect_outline(sprint_btn_rect, arcade.color.WHITE, 2)
        arcade.Text(
            "Sprint",
            sprint_btn_x,
            session_y,
        ...
        checkbox_y = self.center_y - 60
        checkbox_size = 25
        checkbox_x = self.center_x - 150
        checkbox_rect = arcade.XYWH(
            checkbox_x,
            checkbox_y,
            checkbox_size,
            checkbox_size
        )
        arcade.draw_rect_outline(checkbox_rect, arcade.color.WHITE, 2)
        if self.force_refresh:
            arcade.draw_rect_filled(checkbox_rect, arcade.color.WHITE)
        ...
        start_btn_width = 300
        start_btn_height = 60
        start_btn_x = self.center_x
        start_btn_y = self.center_y - 150
        start_btn_rect = arcade.XYWH(
            start_btn_x,
            start_btn_y,
            start_btn_width,
            start_btn_height
        )
        arcade.draw_rect_filled(start_btn_rect, arcade.color.GREEN)
        arcade.draw_rect_outline(start_btn_rect, arcade.color.WHITE, 3)
        arcade.Text(
            "START REPLAY",
            start_btn_x,
            start_btn_y,
        ...
        self.year_btn_rect = year_btn_rect
        self.round_btn_rect = round_btn_rect
        self.race_btn_rect = race_btn_rect
        self.sprint_btn_rect = sprint_btn_rect
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        # Ensure rects are initialized
        if self.year_btn_rect is None:
            return
            
        # Year dropdown
        if self._point_in_rect(x, y, self.year_btn_rect):
            self.year_dropdown_open = not self.year_dropdown_open
            self.round_dropdown_open = False
        elif self.year_dropdown_open:
            # Check if clicking on a year option
            dropdown_y_start = self.year_btn_rect.cy - self.year_btn_rect.h / 2 - 5
            for i, year_option in enumerate(self.years):
                option_y = dropdown_y_start - (i * 35) - 20
                option_rect = arcade.XYWH(
                    self.year_btn_rect.cx,
                    option_y,
                    self.year_btn_rect.w,
                    30
                )
                if self._point_in_rect(x, y, option_rect):
                    self.year = year_option
                    self.year_dropdown_open = False
                    break
            else:
        ...
        # Round dropdown
        elif self._point_in_rect(x, y, self.round_btn_rect):
            self.round_dropdown_open = not self.round_dropdown_open
            self.year_dropdown_open = False
        elif self.round_dropdown_open:
            # Check if clicking on a round option
            dropdown_y_start = self.round_btn_rect.cy - self.round_btn_rect.h / 2 - 5
            for i, round_option in enumerate(self.rounds):
                option_y = dropdown_y_start - (i * 35) - 20
                if option_y > 50:  # Only check if visible
                    option_rect = arcade.XYWH(
                        self.round_btn_rect.cx,
                        option_y,
                        self.round_btn_rect.w,
                        30
                    )
                    if self._point_in_rect(x, y, option_rect):
                        self.round_number = round_option
                        self.round_dropdown_open = False
                        break
            else:
                # Clicked outside, close dropdown
        ...
        elif self._point_in_rect(x, y, self.race_btn_rect):
            self.session_type = 'R'
        elif self._point_in_rect(x, y, self.sprint_btn_rect):
            self.session_type = 'S'
        
        # Checkbox
        elif self._point_in_rect(x, y, self.checkbox_rect):
            self.force_refresh = not self.force_refresh
        
        # Start button
        elif self._point_in_rect(x, y, self.start_btn_rect):
            # Close this window and start the replay
            if self.start_callback:
                self.close()
                self.start_callback(self.year, self.round_number, self.session_type, self.force_refresh)
...
def show_selection_menu(start_callback=None):
    """Show the selection menu window"""
    window = SelectionMenuWindow(start_callback=start_callback)
    arcade.run()